...
(function () {
  ...
  function init() {
    const data = window.YL_LTC_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/yl-ltc.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      ...
      .then((meta) => {
        if (meta.ylLtc) {
          els.metaUpdated.textContent = `資料筆數：${meta.ylLtc.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
