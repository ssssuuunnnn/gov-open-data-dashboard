...
(function () {
  ...
  const map = L.map("map", { preferCanvas: true }).setView([22.99, 121.17], 9);
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    maxZoom: 18,
    attribution: "&copy; OpenStreetMap contributors",
  }).addTo(map);
  const clusterGroup = L.markerClusterGroup({ chunkedLoading: true, disableClusteringAtZoom: 14 });
  ...
  const table = createPagedTable({
    ...
    columns: [
      ...
      { key: "website", label: "相關網址", render: (r) => websiteLink(r.website) },
      { key: "remark", label: "備註", render: (r) => escapeHtml(r.remark) },
    ],
    onRowClick: (row) => {
      if (row.lat && row.lng) {
  ...
  function websiteLink(url) {
    if (!url) return "";
    return `<a href="${escapeHtml(url)}" target="_blank" rel="noopener">官網連結</a>`;
  }
  ...
  function renderStats() {
    ...
    rows.forEach((r) => {
      ...
      if (["1", "2", "3", "4", "5", "6", "7", "8"].every((c) => r.categoryList.includes(c))) fullCount += 1;
    });
  ...
  function renderMap() {
    ...
    state.filtered.forEach((r) => {
      ...
      marker.bindPopup(
        `<strong>${escapeHtml(r.name)}</strong><br/>${escapeHtml(r.address)}<br/>電話：${phoneLink(r.phone) || "-"}<br/>鑑定類別：${escapeHtml(r.categoryText || "-")}`
      );
      markers.push(marker);
    });
  ...
  function renderCharts() {
    ...
    categoryChart = new Chart(document.getElementById("chart-category"), {
      type: "bar",
      data: {
        labels: categoryEntries.map((e) => (e[0] === "dev" ? "發展遲緩" : `第${e[0]}類`)),
  ...
  function init() {
    const data = window.TT_DISABILITY_HOSPITALS_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/tt-disability-hospitals.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      .then((r) => r.json())
      .then((meta) => {
        if (meta.ttDisabilityHospitals) {
          els.metaUpdated.textContent = `資料筆數：${meta.ttDisabilityHospitals.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
