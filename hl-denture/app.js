/* 花蓮縣115年度65歲以上長者假牙補助合約醫療院所 dashboard */
(function () {
  ...
  function init() {
    const data = window.HL_DENTURE_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/hl-denture.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      ...
      .then((meta) => {
        if (meta.hlDenture) {
          els.metaUpdated.textContent = `資料筆數：${meta.hlDenture.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
