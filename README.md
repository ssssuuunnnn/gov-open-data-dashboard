...
以政府開放資料建置的靜態網站，整理五十八個長照/老人福利/身心障礙相關資料集為互動式儀表板，可直接部署於 GitHub Pages。
...
| `hsc-elder-checkup/` | 新竹縣65歲以上老人健康檢查據點 | 新竹縣政府社會處 | 65歲以上長者（含55歲以上原住民）老人健康檢查合約醫療院所名冊（DCAT dataset id 111994，https://data.gov.tw/dataset/8572），共11筆聯絡資料（10家不重複醫院），收錄醫院名稱、電話、傳真、電子郵件、地址、網址、最後更新時間，地址已含「新竹縣」字首可直接解析行政區；原始19列因部分醫院登記多組分機而重複，`build_hsc_elder_checkup()` 依（名稱,地址,電話）分組合併分機（電話顯示「轉 A、B」），使用者提供公告文字寫「9家合約醫院」與實測10家不符，如實依資料集內容呈現；頁面上方另整理115年度預約登記時間、健檢時間、資格條件、檢查項目、如何預約登記、攜帶證件等 Q&A，提供行政區／關鍵字篩選與統計圖表，無經緯度座標 |
...
| `tpe-hospice/` | 安寧療護機構資源查詢 | 臺北市政府衛生局 | 臺北市政府衛生局提供之安寧療護轉介合作醫療院所名單（DCAT dataset id 132385），共85筆，來源CSV欄位為業務組別／特約類別／縣市別／醫事機構名稱／各類病床使用情形網址，BIG5編碼；**實際內容為全國性名單而非僅臺北市機構**，「業務組別」（臺北/北區/中區/南區/高屏/東區）與「特約類別」（醫學中心/區域醫院/地區醫院）欄位為合併儲存格匯出格式，本腳本往下遞補空白列；「縣市別」為數字代碼（如63000000=臺北市），用固定21種代碼對照表轉換為中文縣市名；「各類病床使用情形網址」欄位以JSON陣列字串存放（部分機構如臺北市立聯合醫院含多個院區網址，以換行＋院區名稱標籤區隔，本腳本拆解為`[{label,url},...]`），前端渲染成多個獨立可點擊連結；少數機構名稱前誤植縣市代碼數字（如「63000000立聯合醫院」）為原始資料瑕疵，照原樣顯示不修改。提供業務組別／特約類別／縣市／關鍵字篩選與統計圖表，無地址、無經緯度座標，本頁不含地圖。來源網址無CORS標頭，比照其他資料集用內嵌 js 版本輸出 |
...
| `hl-elder-checkup/` | 花蓮縣免費長者健康檢查醫療院所表 | 花蓮縣衛生局 | 花蓮縣免費長者健康檢查醫療院所表（DCAT dataset id 146136，https://cms.data.gov.tw/dataset/146136），共22筆，來源CSV欄位為醫療院所／電話／鄉鎮別／地址／緯度／經度／補助類型／補助對象／補助內容，與DCAT description一致；使用者原提供衛生局公告PDF附件連結，但DCAT另提供結構化CSV distribution內容更完整，故改採CSV。地址已含完整「花蓮縣OO市/鄉/鎮」字首可直接用 `parse_county_district(fallback_county="花蓮縣")` 解析行政區（分布共13個鄉鎮市：花蓮市、鳳林鎮、玉里鎮、新城鄉、吉安鄉、壽豐鄉、光復鄉、豐濱鄉、瑞穗鄉、富里鄉、秀林鄉、萬榮鄉、卓溪鄉），無typo。緯度／經度已內建於原始資料（WGS84），不需地理編碼或座標轉換。「補助類型」「補助對象」「補助內容」三欄位實測全數22筆皆為相同常數值，仍如實完整輸出但不建立篩選下拉選單。來源網址無CORS標頭，本腳本於伺服器端下載並輸出內嵌JS版本。資料量小（22筆）且已內建座標，加地圖呈現（Leaflet+MarkerCluster+circleMarker，不需抽樣上限），提供鄉鎮別／關鍵字篩選與統計圖表（各鄉鎮院所數） |
...
| `diabetes-care/` | 醫療給付改善方案院所-糖尿病 | 衛生福利部中央健康保險署 | 全國性資料（DCAT dataset id 39263，https://data.gov.tw/dataset/8572），糖尿病醫療給付改善方案試辦院所名單，來源 CSV（https://info.nhi.gov.tw/api/iode0000s01/Dataset?rId=A21030000I-D20004-001）共1,970筆，欄位：醫事機構名稱／醫事機構地址／醫事機構電話／改善方案項目／試辦計畫生效起日／試辦計畫生效迄日，與 DCAT description 一致，欄位數皆一致無缺欄位髒資料。地址已含完整「OO市/縣OO區/鄉/鎮/市」字首，實測全數1,970筆皆可用 `parse_county_district()` 解析出縣市，涵蓋全台22縣市（含連江、金門）；無經緯度座標，故不含地圖。「改善方案項目」欄位實測全數為常數值「糖尿病」，無篩選意義，僅照登輸出不做篩選下拉。「試辦計畫生效起日/迄日」為西元 YYYYMMDD 字串，前端顯示時轉換為 YYYY-MM-DD；迄日實測僅2種值：絕大多數為 `29101231`（代表無期限/持續有效，顯示為「持續有效」），僅1筆為 `20280424`（已知/預定終止日，表格另加註提示），原始欄位值不變更。提供縣市／關鍵字篩選與統計圖表（各縣市院所數長條圖、試辦計畫生效年度累計院所數折線圖）。來源網址無 CORS 標頭，改由 build script 於伺服器端下載，另輸出內嵌 js 版本 |
| `assistive-devices/` | 全台22縣市身心障礙者輔具費用補助懶人包 | 各縣市地方政府（衛生福利部社會及家庭署輔具資源入口網彙整） | 使用者提供衛福部社家署「輔具資源入口網」（newrepat.sfaa.gov.tw）22 縣市「身心障礙者輔具費用補助」公告詳情頁網址，皆為伺服器端渲染純 HTML，`build_assistive_devices()` 逐頁下載並用正規表示式解析固定的 `article-item`/`h4`/`article-word` 結構，取出服務對象、服務內容、申請辦法、業務單位、聯絡電話、相關連結六欄位，另附頁面標題與官方標示之更新日期。欄位不保證每縣市齊全：新北市、新竹市、臺中市、連江縣公告原文缺「服務內容」（非解析遺漏），如實輸出空字串；苗栗縣公告以「申請資格」取代「服務對象」，語意相同已對應合併。「相關連結」欄位輸出 JSON 陣列字串（比照 `build_tpe_hospice()` 的 bedInfoLinks 慣例），連結網址一律轉為絕對網址。澎湖縣：使用者原提供之公告連結已於原網站移除（回傳 HTTP 500），改採內容結構相同、欄位齊全之對應方案頁面「澎湖縣身心障礙生活輔具補助服務（含代償墊付資訊）」。此為 22 縣市橫向比較的公告文字內容頁而非機構清單表格，故不提供地圖／篩選表格，改用縣市切換按鈕（22 個縣市內容全部寫入 HTML，僅用 `hidden` 屬性切換顯示，確保 SEO 可完整索引、無 JS 時仍可捲動閱讀），支援 `?county=` 網址參數深連結。來源網址無 CORS 標頭，比照專案慣例用內嵌 js 版本輸出，可執行 `python3 scripts/build_data.py assistive-devices` 重新抓取更新 |
...
hsc-denture/app.js
hsc-elder-checkup/index.html 新竹縣65歲以上老人健康檢查據點儀表板（Chart.js 圖表 + 篩選表格，無地圖；
                       頁面上方另有預約登記時間/健檢時間/資格條件/檢查項目/攜帶證件 Q&A 說明）
hsc-elder-checkup/app.js
...
kcg-elder-checkup/index.html 高雄市老人健檢醫療院所儀表板（Chart.js 圖表 + 篩選表格，無地圖；
                       頁面上方另有服務對象/健檢項目/檢查日期名額/獎勵方案等公告 Q&A 說明卡片）
...
hl-elder-checkup/index.html 花蓮縣免費長者健康檢查醫療院所表地圖儀表板（Leaflet 地圖 + Chart.js
                       圖表 + 篩選表格）
hl-elder-checkup/app.js
diabetes-care/index.html 全國糖尿病醫療給付改善方案試辦院所查詢儀表板（Chart.js 圖表 + 篩選表格，
                       無地圖）
diabetes-care/app.js
assistive-devices/index.html 全台22縣市身心障礙者輔具費用補助懶人包（縣市切換按鈕 + 22 個縣市
                       內容區塊，皆直接寫入 HTML 僅切換顯示/隱藏，無地圖無篩選表格）
assistive-devices/app.js
...
data/hsc-denture.js   同上資料的內嵌 JS 版本（window.HSC_DENTURE_DATA），供 hsc-denture 頁面以
                       <script> 標籤直接載入，因來源網址無 CORS 標頭，不透過 fetch()
data/hsc-elder-checkup.json 新竹縣65歲以上老人健康檢查據點資料（由 scripts/build_data.py 自動下載
                       新竹縣政府開放資料 JSON 並合併同機構多組分機後產生）
data/hsc-elder-checkup.js 同上資料的內嵌 JS 版本（window.HSC_ELDER_CHECKUP_DATA），供
                       hsc-elder-checkup 頁面以 <script> 標籤直接載入，因來源網址無 CORS 標頭，
                       不透過 fetch()
...
data/tpe-hospice.json  安寧療護機構資源查詢（由 scripts/build_data.py 下載 data.taipei BIG5 CSV
                       產生，實際內容為全國性名單、來源網址無 CORS 標頭，無經緯度座標）
data/tpe-hospice.js    同上資料的內嵌 JS 版本（window.TPE_HOSPICE_DATA），供 tpe-hospice 頁面以
                       <script> 標籤直接載入
...
data/tt-disability-hospitals.json  115年臺東縣身心障礙鑑定醫院（由 scripts/build_data.py 下載
...
data/nt-disability-hospitals.json  南投縣身心障礙鑑定醫院（由 scripts/build_data.py 下載
...
data/kcg-hospitals.js  同上資料的內嵌 JS 版本（window.KCG_HOSPITALS_DATA），供 kcg-hospitals
                       頁面以 <script> 標籤直接載入，因來源網址無 CORS 標頭，不透過 fetch()
data/kcg-elder-checkup.json  高雄市老人健檢醫療院所資料（由 scripts/build_data.py 產生）
data/kcg-elder-checkup.js  同上資料的內嵌 JS 版本（window.KCG_ELDER_CHECKUP_DATA），供
                       kcg-elder-checkup 頁面以 <script> 標籤直接載入，因來源網址無 CORS 標頭，
                       不透過 fetch()
data/ntpc-elder-checkup.json  新北市長者健康檢查醫療院所資料（由 scripts/build_data.py 產生）
...
data/hl-elder-checkup.json  花蓮縣免費長者健康檢查醫療院所表資料（由 scripts/build_data.py 產生）
data/hl-elder-checkup.js  同上資料的內嵌 JS 版本（window.HL_ELDER_CHECKUP_DATA），供
                       hl-elder-checkup 頁面以 <script> 標籤直接載入，因來源網址無 CORS 標頭，
                       不透過 fetch()
data/diabetes-care.json  醫療給付改善方案院所-糖尿病資料（由 scripts/build_data.py 產生）
data/diabetes-care.js  同上資料的內嵌 JS 版本（window.DIABETES_CARE_DATA），供
                       diabetes-care 頁面以 <script> 標籤直接載入，因來源網址無 CORS 標頭，
                       不透過 fetch()
data/assistive-devices.json  全台22縣市身心障礙者輔具費用補助懶人包資料（由 scripts/build_data.py
                       逐頁下載22個公告詳情頁 HTML 解析產生）
data/assistive-devices.js  同上資料的內嵌 JS 版本（window.ASSISTIVE_DEVICES_DATA），供
                       assistive-devices 頁面以 <script> 標籤直接載入，因來源網址無 CORS 標頭，
                       不透過 fetch()
...
python3 scripts/build_data.py hsc-elder-checkup   # 只重新產生新竹縣65歲以上老人健康檢查據點
...
python3 scripts/build_data.py tpe-hospice  # 只重新產生安寧療護機構資源查詢
...
python3 scripts/build_data.py hl-elder-checkup    # 只重新產生花蓮縣免費長者健康檢查醫療院所表
...
python3 scripts/build_data.py diabetes-care       # 只重新產生醫療給付改善方案院所-糖尿病
python3 scripts/build_data.py assistive-devices   # 只重新產生全台22縣市身心障礙者輔具費用補助懶人包
