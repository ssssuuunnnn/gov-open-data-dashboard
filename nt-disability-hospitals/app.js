...
(function () {
  ...
  function injectItemListJsonLd(rows) {
    const itemListElement = rows.map((r, i) => {
      ...
      if (r.categoryList && r.categoryList.length) {
        item.medicalSpecialty = r.categoryList
  ...
  function renderStats() {
    ...
    rows.forEach((r) => {
      ...
      if (r.categoryList.filter((c) => c !== "dev").length >= 8) fullCount += 1;
    });
  ...
  function init() {
    const data = window.NT_DISABILITY_HOSPITALS_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/nt-disability-hospitals.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      .then((r) => r.json())
      .then((meta) => {
        if (meta.ntDisabilityHospitals) {
          els.metaUpdated.textContent = `資料筆數：${meta.ntDisabilityHospitals.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
