...
(function () {
  ...
  // 動態插入 ItemList 結構化資料（列出全部機構），供搜尋引擎理解本頁機構名單內容；因資料以 JS
  ...
  function init() {
    const data = window.HCCG_DISABILITY_HOSPITALS_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/hccg-disability-hospitals.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      .then((r) => r.json())
      .then((meta) => {
        if (meta.hccgDisabilityHospitals) {
          els.metaUpdated.textContent = `資料筆數：${meta.hccgDisabilityHospitals.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
