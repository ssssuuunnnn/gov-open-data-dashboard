...
(function () {
  ...
  const table = createPagedTable({
    ...
    columns: [
      ...
      { key: "name", label: "機構名稱" },
      { key: "subtype", label: "機構型態（推斷）", render: (r) => escapeHtml(r.subtype) },
  ...
  // 電話欄位可能合併多支號碼（以「、」分隔）且偶夾帶「/」分隔的多組號碼，僅第一組轉為 tel: 連結，
  // 顯示文字保留原始格式（含全部合併的電話）。
  ...
  // 地址已含完整「桃園市OO區」字首，可直接轉成 Google Maps 搜尋連結。
  ...
  function renderCharts() {
    ...
    categoryChart = new Chart(document.getElementById("chart-category"), {
      ...
      data: {
        ...
        datasets: [{ data: categoryEntries.map((e) => e[1]), backgroundColor: ["#1f6f5c", "#e07a2c", "#3763c9"] }],
      },
  ...
  els.subtype.addEventListener("change", applyFilters);
  ...
  function init() {
    const data = window.TYC_HOSPICE_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/tyc-hospice.js</div>`;
      return;
    }
    ...
    const subtypes = Array.from(new Set(state.all.map((r) => r.subtype))).filter(Boolean).sort((a, b) => a.localeCompare(b, "zh-Hant"));
    populateSelect(els.subtype, subtypes);
    ...
    fetch("../data/meta.json")
      ...
      .then((meta) => {
        if (meta.tycHospice) {
          els.metaUpdated.textContent = `資料筆數：${meta.tycHospice.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
