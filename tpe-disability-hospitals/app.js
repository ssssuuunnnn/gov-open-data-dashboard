/* 臺北市身心障礙鑑定指定醫院聯絡方式一覽表 dashboard */
(function () {
  ...
  function init() {
    const data = window.TPE_DISABILITY_HOSPITALS_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/tpe-disability-hospitals.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      .then((r) => r.json())
      .then((meta) => {
        if (meta.tpeDisabilityHospitals) {
          els.metaUpdated.textContent = `資料筆數：${meta.tpeDisabilityHospitals.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
