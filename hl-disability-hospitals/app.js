...
(function () {
  ...
  const map = L.map("map", { preferCanvas: true }).setView([23.6, 121.4], 9);
    ...
    const itemListElement = rows.map((r, i) => {
      ...
      if (r.address) {
        item.address = {
          ...
          addressLocality: r.district || undefined,
          addressRegion: r.county || "花蓮縣",
          addressCountry: "TW",
        };
      }
  ...
  function renderStats() {
    ...
    els.statWebsite.textContent = websiteCount.toLocaleString();
  }
  ...
  function renderMap() {
    ...
    state.filtered.forEach((r) => {
      ...
      marker.bindPopup(
        `<strong>${escapeHtml(r.name)}</strong><br/>${escapeHtml(r.address)}<br/>電話：${phoneLink(r.phone) || "-"}`
      );
      markers.push(marker);
    });
  ...
  function init() {
    const data = window.HL_DISABILITY_HOSPITALS_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/hl-disability-hospitals.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      .then((r) => r.json())
      .then((meta) => {
        if (meta.hlDisabilityHospitals) {
          els.metaUpdated.textContent = `資料筆數：${meta.hlDisabilityHospitals.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
