/* 臺中市115年度65歲以上銀髮族假牙裝置補助計畫合約院所 dashboard */
(function () {
  ...
  // 沒有 place_id（一次性抓取時查無對照資料或人工核對後排除）則只顯示數字或「-」，不猜測連結。
  ...
  // 地址欄位本身已含「臺中市」字首（原始資料格式，與 chc-denture／hsc-denture 不同），
  // 可直接用於 Google Maps 搜尋連結，不需另外補縣市前綴。
  ...
  function init() {
    const data = window.TC_DENTURE_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/tc-denture.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      ...
      .then((meta) => {
        if (meta.tcDenture) {
          els.metaUpdated.textContent = `資料筆數：${meta.tcDenture.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
