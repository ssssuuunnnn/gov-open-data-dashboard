/* 宜蘭縣中低收入戶老人假牙裝置補助特約牙醫醫療院所 dashboard */
(function () {
  ...
  function init() {
    const data = window.YL_DENTURE_DATA;
    if (!data) {
      document.getElementById("table-container").innerHTML = `<div class="loading">資料載入失敗：找不到內嵌資料 data/yl-denture.js</div>`;
      return;
    }
    ...
    fetch("../data/meta.json")
      ...
      .then((meta) => {
        if (meta.ylDenture) {
          els.metaUpdated.textContent = `資料筆數：${meta.ylDenture.count.toLocaleString()}　資料整理時間：${new Date(meta.generatedAt).toLocaleString("zh-Hant-TW")}`;
        }
      })
      .catch(() => {});
  }
